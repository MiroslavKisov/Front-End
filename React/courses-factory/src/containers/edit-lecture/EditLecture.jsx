...
const EditLecture = (props) => {
  const {resetRedirectPath, loading, redirectPath, editLecture} = props;
  const {lectureId} = props.match.params;
  const {courseId} = props.match.params;

  const [name, setName] = useState(sessionStorage.getItem('lectName'));
  const [nameValid, setNameValid] = useState(false);

  const [description, setDescription] = useState(sessionStorage.getItem('lectDesc'));
  const [descriptionValid, setDescriptionValid] = useState(false);

  const[video] = useState(sessionStorage.getItem('lectVideo'));
  ...
  useEffect(() => {
    ...
    //Validate the lecture description.
  ...
  const submitHandler = (event) => {
    ...
    editLecture(lectureId, name, description, video, courseId);
  }
  ...
  return (
    <div>
      ...
      {loading ? <Spinner /> :
      <div>
        <h1 className="text-center m-6 text-4xl text-teal-500">Edit Lecture</h1>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    editLecture: (
      ...
      video,
      courseId
    ) => dispatch(lecturesActions.editLecture(
      lectureId,
      name,
      description,
      video,
...
export default connect(mapStateToProps, mapDispatchToProps)(EditLecture);